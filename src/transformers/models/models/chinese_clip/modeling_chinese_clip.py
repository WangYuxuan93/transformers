...
class ChineseCLIPForVQA(ChineseCLIPPreTrainedModel):
    config_class = ChineseCLIPConfig
    _tied_weights_keys = ["text_decoder.cls.predictions.decoder.bias"]

    def __init__(self, config: ChineseCLIPConfig,decoder_model_path):
        super().__init__(config)

        self.vision_model = ChineseCLIPVisionModel(config.vision_config)

        self.text_encoder = ChineseCLIPTextModel(config.text_config, add_pooling_layer=False)
    ...
    def forward(
        ...
        question_attention_mask: Optional[torch.LongTensor] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        ...
        if self.training:
            ...
            if labels is None and answer_input_ids is None:
                raise ValueError(
                    "Either `answer_input_ids` or `labels` should be passed when calling `forward` with"
                    " `BlipForQuestionAnswering`. if you are training the model make sure that `labels` is passed, if you"
                    " are using the model for inference make sure that `answer_input_ids` is passed or call `generate`"
                )
            ...
            if labels is not None and answer_input_ids is None:
            ...
            log_probs_sum,topk_ids, topk_probs = self.rank_answer(question_embeds[0], question_attention_mask, 
...
class ChineseCLIPForVD(ChineseCLIPPreTrainedModel):
    config_class = ChineseCLIPConfig
    _tied_weights_keys = ["text_decoder.cls.predictions.decoder.bias"]
    ...
    def forward(
        ...
        if self.training:
            ...
            log_probs_sum,topk_ids, topk_probs = self.rank_answer(question_embeds[0], text_attention_mask, 
...
class ChineseCLIPForVG(ChineseCLIPPreTrainedModel):
    config_class = ChineseCLIPConfig
    _tied_weights_keys = ["text_decoder.cls.predictions.decoder.bias"]
